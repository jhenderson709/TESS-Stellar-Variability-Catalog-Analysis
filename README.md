...
By studying these stellar vibrations, we can measure a star’s age, size, and even how it evolves over time. This helps us understand not just individual stars but also the broader life cycles of stars across the universe.


 This work, as it relates to asteroseismology, is a result of identifying and analyzing variable light signatures in the light curves of stars observed by the TESS spacecraft; we used differential photometry to obtain light curves and performed Fourier transforms on the data to proceed with analysis. The Fourier transform is a mathematical model that, using integral transform, takes a general function and outputs it as a continuous integral of sines and cosines with a spectrum of possible frequencies [11]. For our intents and purposes, this enables the conversion of time-series photometry from the time domain to the frequency domain; light data from stars in the form of amplitude as a function of time are run through a Fourier transform and outputted as amplitude as a function of frequency. If periodicity is present in the light curve, an amplitude peak – a pulsation candidate – will emerge in the amplitude-frequency domain that corresponds to the frequency of said periodicity. If a given peak within a spectrum has been vetted for frequency and contamination considerations and is determined to be of sufficient amplitude, the peak can be called a pulsation and the star to which it belongs can be distinguished as a variable star.
There are two general groups of variable stars: intrinsic and extrinsic [12]. Intrinsic variable stars include pulsators and eruptive stars. In either case, a variable brightness signature is caused by activity that is intrinsic to the star; pulsators are physically changing in size/brightness periodically, and eruptive stars undergo cataclysmic events such as novae or supernovae that output an extremely intense transient brightness. Extrinsic variable stars are those which exhibit a variable brightness due to an external influence such as the presence of an eclipsing binary or geometric binary. Under these conditions, stars eclipse one another in the plane of observation causing a variable brightness that corresponds to orbital period.
